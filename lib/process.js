  ...
  targetPaths.forEach(function (pathObj, ndx, arr) {
    ...
    if (ops.length !== 0) {
      data.push({
        ...
        'operations': ops,
        'camelCased': pathObj.path.replace(/[\/{}]+(.)/g, (_, v) => v.toUpperCase())
      })
    }
  })
  ...
  parentPath.getOperations().forEach(function (op, ndx, arr) {
    ...
    if (transactions.length !== 0) {
      ops.push({
        ...
        'camelCased': parentPath.path.replace(/[\/{}]+(.)/g, (_, v) => v.toUpperCase())+op.method[0].toUpperCase()+op.method.substring(1)
      })
    }
  })
