...
var getSample = require('sway/lib/helpers').getSample
...
function process(api, options) {
  var server0URL = api.servers != undefined ? new URL(api.servers[0].url) : undefined
  var processedSpec = {
    'host': (options.host !== undefined ? options.host : (api.host !== undefined
      ? api.host : (api.servers !== undefined ? server0URL.host : 'localhost:5000'))),
    'scheme': (options.scheme !== undefined ? options.scheme : (api.schemes
      !== undefined ? api.schemes[0] : (api.servers !== undefined ? server0URL.protocol.slice(0,-1) : 'http'))),
    'basePath': (api.basePath !== undefined ? api.basePath : (api.servers !== undefined ? server0URL.pathname : '')),
  ...
  targetPaths.forEach(function (pathObj, ndx, arr) {
    ...
    if (ops.length !== 0) {
      data.push({
        ...
        'operations': ops,
        'camelCased': pathObj.path.replace(/[\/{}]+(.)?/g, (_, v) => v ? v.toUpperCase() : '')
      })
    }
  })
  ...
  parentPath.getOperations().forEach(function (op, ndx, arr) {
    ...
    if (transactions.length !== 0) {
      ops.push({
        ...
        'camelCased': parentPath.path.replace(/[\/{}]+(.)?/g, (_, v) => v ? v.toUpperCase() : '')+op.method[0].toUpperCase()+op.method.substring(1)
      })
    }
  })
...
function processTransactions(api, parentOp, parentPath, topLevel, options) {
  ...
  parentOp.getResponses().forEach(function (res, ndx, arr) {
    ...
    // count multiple status code entries
    var count = 1;
    if (options.customValues
      && options.customValues[parentPath.path]
      && options.customValues[parentPath.path][parentOp.method]
      && options.customValues[parentPath.path][parentOp.method][res.statusCode]
      && Array.isArray(options.customValues[parentPath.path][parentOp.method][res.statusCode])) {
      count = options.customValues[parentPath.path][parentOp.method][res.statusCode].length;
    }

    // generate transactions for all custom values
      ...
      transactions.push({
        ...
        'headers': merge2(params.headers,
          processHeaders(res.statusCode, parentOp, parentPath, topLevel,
            options, index)),
        'camelCased': parentPath.path.replace(/[\/{}]+(.)?/g, (_, v) => v ? v.toUpperCase() : '')+parentOp.method[0].toUpperCase()+parentOp.method.substring(1)+expected.statusCode
      })
    }
  })
 ...
 */
function processParams(code, op, path, options, index) {
  var opCustomQuery = lookupCustomQueryValue(code, op.method, path.path, options, index);
  var params = {
    'body': {},
    'query': opCustomQuery || {},
    'formData': {},
  ...
  if (op.requestBody !== undefined &&
      op.definitionFullyResolved.requestBody.content['application/json'] && 
      op.definitionFullyResolved.requestBody.content['application/json'].schema) {
    var customValue = lookupCustomValue('body', 'body', code, op.method,
      path.path, options, index)
    // Skip optional parameter with specifically nulled custom value or in-line example
    if (customValue === null) {
      return
    } 
    params.body = customValue !== undefined ? customValue : 
      op.definitionFullyResolved.requestBody.content['application/json'] !== undefined ?
      getSample(op.definitionFullyResolved.requestBody.content['application/json'].schema) : {}
  }

  if (op.requestBody !== undefined &&
    op.definitionFullyResolved.requestBody.content['application/x-www-form-urlencoded'] &&
    op.definitionFullyResolved.requestBody.content['application/x-www-form-urlencoded'].schema) {
    Object.keys(op.definitionFullyResolved.requestBody.content['application/x-www-form-urlencoded'].schema.properties).forEach(function (param, ndx, arr) {
      var customValue = lookupCustomValue(param, 'formData', code, op.method,
      path.path, options, index)
      // Skip optional parameter with specifically nulled custom value or in-line example
      if (customValue === null) {
        return
      ...
      var res = {}
      res[param] = customValue !== undefined ? customValue : 
      op.definitionFullyResolved.requestBody.content['application/x-www-form-urlencoded'].schema.properties[param].example
      Object.assign(params.formData, res)
    })
  }
 ...
 * @return {any}
 */
function lookupCustomValue(name, location, code, op, path, options, index) {
  ...
  if (options.customValues) {
    ...
    do {
      ...
      curr = Array.isArray(curr[levels[ndx]]) ? curr[levels[ndx]][index] : curr[levels[ndx]];
 ...
 * @return {any}
 */
function lookupCustomQueryValue(code, op, path, options, index) {
  var levels = [path, op, code];
  var val = undefined;
 ...
 * @return {object}
 */
function processHeaders(responseCode, op, path, top, options, index) {
  var headers = {};
  ...
  if (options.consumes) { // a specific content-type was targeted
    ...
    headers['Content-Type'] = op.definitionFullyResolved.consumes[0];
  } else if (op.definitionFullyResolved.requestBody && op.definitionFullyResolved.requestBody.content) {
    headers['Content-Type'] = Object.keys(op.definitionFullyResolved.requestBody.content)[0];
  ...
  if (options.produces) {
    ...
    headers['Accept'] = op.definitionFullyResolved.produces[0];
  } else if (op.definitionFullyResolved.responses && op.definitionFullyResolved.responses[responseCode] && op.definitionFullyResolved.responses[responseCode].content) {
    headers['Accept'] = Object.keys(op.definitionFullyResolved.responses[responseCode].content)[0];
 ...
 */
function processResponse(res, op, path, options, index) {
  var expected = {
  ...
  if (options.customValues
    ...
    var statusCodeOptions = options.customValues[path][op][res.statusCode];
    if (Array.isArray(statusCodeOptions)) {
      statusCodeOptions = statusCodeOptions[index];
    }

    if (statusCodeOptions.response) {

      expected['res'] = statusCodeOptions.response
  ...
  var res = options.samples ? res.getSample()
    ...
    ? res.definitionFullyResolved.schema
    : res.definitionFullyResolved.content && res.definitionFullyResolved.content['application/json'] && res.definitionFullyResolved.content['application/json'].schema
    ? res.definitionFullyResolved.content['application/json'].schema
    : undefined
  if (res !== undefined) {
    expected['res'] = res
  }
